...
def calculateHypercube():
    ...
    data4radarplots = {'radarLabels': radarLabels, 'finalHyperVolume': finalHyperVolume}
    df = pd.DataFrame(data4radarplots)
    aggregatedColumns = ['finalHyperVolume']
    mean_df = df.groupby('radarLabels')[aggregatedColumns].mean()
    mean_df.reset_index(inplace=True)
    std_df = df.groupby('radarLabels')[aggregatedColumns].std(ddof=1)
    std_df.reset_index(inplace=True)

    # df.groupby('name')['fsColonyOptTime','oneColonyOptTime', 'selectedFromParetoOptTime', 'fsColonyNetTime', 'oneColonyNetTime', 'selectedFromParetoNetTime'].sem()

    radarLabels = mean_df["radarLabels"].values.tolist()
    hyperVolume = mean_df["finalHyperVolume"].values.tolist()
    std_radarLabels = std_df["radarLabels"].values.tolist()
    std_hyperVolume = std_df["finalHyperVolume"].values.tolist()

    print("Values of the average hypervolumen and the standard deviation")
    for i in range(len(radarLabels)):
        j=0
        while j<len(std_radarLabels) and std_radarLabels[j]!=radarLabels[i]:
            j+=1
        print(radarLabels[i]+"   &   "+str(hyperVolume[i]) + " ("+str(std_hyperVolume[j])+")")
...
def drawAllRadarPlots():
    ...
    oneColonyOptTime = radar2['oneColonyOptTime']
    selectedFromParetoOptTime = radar2['selectedFromParetoOptTime']
    fsColonyNetTime = radar2['fsColonyNetTime']
    oneColonyNetTime = radar2['oneColonyNetTime']
    selectedFromParetoNetTime = radar2['selectedFromParetoNetTime']

    fsColonyDistance = radar2['fsColonyDistance']
    oneColonyDistance = radar2['oneColonyDistance']
    selectedFromParetoDistance = radar2['selectedFromParetoDistance']
    ...
    data4radarplots = {'radarLabels': radarLabels, 'fsColonyOptTime': fsColonyOptTime,
                       'oneColonyOptTime': oneColonyOptTime, 'selectedFromParetoOptTime': selectedFromParetoOptTime,
                       'fsColonyNetTime': fsColonyNetTime, 'oneColonyNetTime': oneColonyNetTime,
                       'selectedFromParetoNetTime': selectedFromParetoNetTime,'fsColonyDistance': fsColonyDistance,
                       'oneColonyDistance': oneColonyDistance,
                       'selectedFromParetoDistance': selectedFromParetoDistance}
    df = pd.DataFrame(data4radarplots)
    aggregatedColumns = ['fsColonyOptTime', 'oneColonyOptTime', 'selectedFromParetoOptTime', 'fsColonyNetTime',
                         'oneColonyNetTime', 'selectedFromParetoNetTime', 'fsColonyDistance',
                         'oneColonyDistance', 'selectedFromParetoDistance']
    mean_df = df.groupby('radarLabels')[aggregatedColumns].mean()
    mean_df.reset_index(inplace=True)
    std_df = df.groupby('radarLabels')[aggregatedColumns].std(ddof=1)
    std_df.reset_index(inplace=True)
    ...
    oneColonyOptTime = mean_df["oneColonyOptTime"].values.tolist()
    ...
    oneColonyNetTime = mean_df["oneColonyNetTime"].values.tolist()
    selectedFromParetoNetTime = mean_df["selectedFromParetoNetTime"].values.tolist()
    fsColonyDistance = mean_df["fsColonyDistance"].values.tolist()
    oneColonyDistance = mean_df["oneColonyDistance"].values.tolist()
    selectedFromParetoDistance = mean_df["selectedFromParetoDistance"].values.tolist()


    std_radarLabels = std_df["radarLabels"].values.tolist()
    std_fsColonyOptTime = std_df["fsColonyOptTime"].values.tolist()
    std_oneColonyOptTime = std_df["oneColonyOptTime"].values.tolist()
    ...
    std_oneColonyNetTime = std_df["oneColonyNetTime"].values.tolist()
    std_selectedFromParetoNetTime = std_df["selectedFromParetoNetTime"].values.tolist()

    std_fsColonyDistance = std_df["fsColonyDistance"].values.tolist()
    std_oneColonyDistance = std_df["oneColonyDistance"].values.tolist()
    std_selectedFromParetoDistance = std_df["selectedFromParetoDistance"].values.tolist()
    ...
    ind1 = copy.copy(oneColonyDistance)
    ind1.append(oneColonyDistance[0])
    series2Plot.append(ind1)

    std_ind1 = copy.copy(std_oneColonyDistance)
    std_ind1.append(std_oneColonyDistance[0])
    std_series2Plot.append(std_ind1)

    ind2 = copy.copy(fsColonyDistance)
    ind2.append(fsColonyDistance[0])
    series2Plot.append(ind2)

    std_ind2 = copy.copy(std_fsColonyDistance)
    std_ind2.append(std_fsColonyDistance[0])
    std_series2Plot.append(std_ind2)

    ind3 = copy.copy(selectedFromParetoDistance)
    ind3.append(selectedFromParetoDistance[0])
    series2Plot.append(ind3)

    std_ind3 = copy.copy(std_selectedFromParetoDistance)
    std_ind3.append(std_selectedFromParetoDistance[0])
    std_series2Plot.append(std_ind3)

    paletteValues = [0, 1, 2]
    radarTitle = 'Euclidean distance to origin'
    theLabels = ["one-colony", "fixed-size", "smallED-GA"]
    fileName2Store = "radarDistance.pdf"

    plotRadarFigures(radarLabels, series2Plot, std_series2Plot, paletteValues, radarTitle, theLabels, fileName2Store,with_Error_Bars=False, with_Bars=False)
    ...
    fileName2Store = "radarDistanceOnly2.pdf"
    ...
    ind1 = copy.copy(oneColonyOptTime)
    ind1.append(oneColonyOptTime[0])
    series2Plot.append(ind1)

    std_ind1 = copy.copy(std_oneColonyOptTime)
    std_ind1.append(std_oneColonyOptTime[0])
    std_series2Plot.append(std_ind1)
    ...
    ind1 = copy.copy(oneColonyNetTime)
    ind1.append(oneColonyNetTime[0])
    series2Plot.append(ind1)

    std_ind1 = copy.copy(std_oneColonyNetTime)
    std_ind1.append(std_oneColonyNetTime[0])
    std_series2Plot.append(std_ind1)
        ...
        for repNum in configuration.rangeOfExperimentsRepetitions:
            ...
            for i in range(configuration.numberGenerations):
                ...
                for j in range(len(oneIterationData['all_paretox'][i])):
                    ...
                    paretoMaxX = max(paretoMaxX, x_value)
                    paretoMaxY = max(paretoMaxY, y_value)
...
def calculateSmallestGenerationDomination2():
    # ******************************************************************************************
    #   Calculation of the generation number where the pareto set of the GA dominates the two control solutions
    # This is an independent code that can be executed isolated, since the data needed is readed from a file
    # that was generated in the whole previous execution of all this  script.
    ...
    for oneExperiment in configuration.experiments2execute:
        ...
        for repNum in configuration.rangeOfExperimentsRepetitions:
            ...
            smallestGenerationDominates = MYMAXVALUE
    ...
    data4TableAnalysis = {'experimentLabels': experimentLabels,
                          'experimentSmallestGeneration': experimentSmallestGeneration}
    df = pd.DataFrame(data4TableAnalysis)
    df = df[df.experimentSmallestGeneration != MYMAXVALUE]
    aggregatedColumns = ['experimentSmallestGeneration']
    mean_df = df.groupby('experimentLabels')[aggregatedColumns].mean()
    mean_df.reset_index(inplace=True)
    std_df = df.groupby('experimentLabels')[aggregatedColumns].std()
    std_df.reset_index(inplace=True)
    total_df = df.groupby('experimentLabels')[aggregatedColumns].count()
    total_df.reset_index(inplace=True)

    experimentLabels = mean_df["experimentLabels"].values.tolist()
    experimentSmallestGenerationValues = mean_df["experimentSmallestGeneration"].values.tolist()
    std_experimentLabels = std_df["experimentLabels"].values.tolist()
    std_experimentSmallestGeneration = std_df["experimentSmallestGeneration"].values.tolist()
    total_experimentLabels = total_df["experimentLabels"].values.tolist()
    total_experimentSmallestGeneration = total_df["experimentSmallestGeneration"].values.tolist()


    print("Values of the average hypervolumen and the standard deviation")
    for i in range(len(experimentLabels)):
        j=0
        while j<len(std_experimentLabels) and std_experimentLabels[j]!=experimentLabels[i]:
            j+=1
        k=0
        while k<len(total_experimentLabels) and total_experimentLabels[k]!=experimentLabels[i]:
            k+=1
        print(str(experimentLabels[i])+"   &   "+str(experimentSmallestGenerationValues[i]) + " ("+str(std_experimentSmallestGeneration[j])+") in "+str(total_experimentSmallestGeneration[j])+" samples")
...
def getParetoComparativeMetrics(oneIterationData):
    ...
    for i in range(len(oneIterationData['all_paretox'])):
        ...
        paretoMaxXnoNormalized = -1.0
        paretoMaxYnoNormalized = -1.0
        paretoMinXnoNormalized = float('inf')
        paretoMinYnoNormalized = float('inf')
        ...
        for j in range(len(oneIterationData['all_paretox'][i])):
            ...
            paretoMinXnoNormalized = min(paretoMinXnoNormalized, x_value)
            paretoMinYnoNormalized = min(paretoMinYnoNormalized, y_value)

            paretoMaxXnoNormalized = max(paretoMaxXnoNormalized, x_value)
            paretoMaxYnoNormalized = max(paretoMaxYnoNormalized, y_value)
...
def plotRadarFigures(radarLabels, series2Plot, std_series2Plot, paletteValues, radarTitle, theLabels, fileName2Store,y_limit=None, with_Error_Bars=True, with_Bars=False):
...
#            errorSerieMin.append(series2Plot[i][j]*0.9)
        ...
        if with_Bars:
            ax.bar(angles, series2Plot[i], linewidth=1, color=my_palette(paletteValues[i]), linestyle='solid',
                    label=theLabels[i])
        else:
...
def getDistanceToReferencePoint(minX,minY,maxX,maxY,xval,yval):
#Caculate the disntance to the point (0,0), the origin of coordinates
    diffX = maxX - minX
    diffY = maxY - minY
    x_value = (xval - minX) / diffX
    y_value = (yval - minY) / diffY
    distanceToCeroCero = math.sqrt(x_value ** 2 + y_value ** 2)
    return distanceToCeroCero
...
oneColonyNetTime = list()
selectedFromParetoNetTime = list()

fsColonyOptTime = list()
oneColonyOptTime = list()
selectedFromParetoOptTime = list()

fsColonyDistance = list()
oneColonyDistance = list()
selectedFromParetoDistance = list()
...
for oneExperiment in configuration.experiments2execute:
    ...
    for repNum in configuration.rangeOfExperimentsRepetitions:
        ...
        oneColonyOptTime.append(controlCases['ot1c'])
        selectedFromParetoOptTime.append(meanCloserTo00X[-1])

        fsColonyNetTime.append(controlCases['rtCc'])
        oneColonyNetTime.append(controlCases['rt1c'])
        selectedFromParetoNetTime.append(meanCloserTo00Y[-1])

        #x are optimization time and y are response time
        fsColonyDistanceValue = getDistanceToReferencePoint(minX, minY, maxX, maxY, controlCases['otCc'], controlCases['rtCc'])
        oneColonyDistanceValue = getDistanceToReferencePoint(minX, minY, maxX, maxY, controlCases['ot1c'], controlCases['rt1c'])

        fsColonyDistance.append(fsColonyDistanceValue)
        oneColonyDistance.append(oneColonyDistanceValue)
        selectedFromParetoDistance.append(meanCloserTo00[-1])
...
radar['oneColonyOptTime'] = oneColonyOptTime
radar['selectedFromParetoOptTime'] = selectedFromParetoOptTime
radar['fsColonyNetTime'] = fsColonyNetTime
radar['oneColonyNetTime'] = oneColonyNetTime
radar['selectedFromParetoNetTime'] = selectedFromParetoNetTime
radar['fsColonyDistance'] = fsColonyDistance
radar['oneColonyDistance'] = oneColonyDistance
radar['selectedFromParetoDistance'] = selectedFromParetoDistance
...
drawAllRadarPlots()
calculateSmallestGenerationDomination2()
calculateHypercube()
